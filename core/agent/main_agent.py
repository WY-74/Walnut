...
class MainAgent(BaseAgent):
    ...
    def handle_action(self, runner: RunTime, tool_manager: ToolManager, run_id: str) -> Callable:
        async def handler(action: ActionPayload):
            if action.tool_call and len(action.tool_call) == 1:
                step_result: PlainText = await self.run_single_step(action.tool_call[0], runner, tool_manager, run_id)
                ...
                for idx, r in enumerate(raw_result):
                    ...
                    _raw_results.append(target + "-> " + r.result)
    ...
    async def run_single_step(self, tool: Tool, runner: RunTime, tool_manager: ToolManager, run_id: str) -> PlainText:
        name = tool.name
        ...
        step_result: PlainText = await self.toolcall_agent.run(
            query=query,
            runner=runner,
            message=Message(),
            tool_manager=tool_manager,
